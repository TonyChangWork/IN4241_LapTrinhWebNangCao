    ...
    <div style={{ padding:"40px", maxWidth:"700px", margin:"0 auto" }}>
      ...
      {cart.length === 0 && (
        <div style={{ textAlign:"center", marginTop:"60px" }}>
          ...
          <Link to="/"><button style={{ marginTop:"16px", padding:"10px 24px" }}>Tiếp tục mua sắm</button></Link>
        </div>
      )}

      {cart.map(item => (
        <div key={item.id} style={{ display:"flex", gap:"20px", alignItems:"center", marginBottom:"16px", padding:"12px", border:"1px solid #eee", borderRadius:"8px" }}>
          ...
          <button onClick={() => removeItem(item.id)} style={{ color:"red", background:"none", border:"none", cursor:"pointer", fontSize:"18px" }}>✕</button>
        </div>
      ))}
